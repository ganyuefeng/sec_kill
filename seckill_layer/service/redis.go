...
func RunProcess() (err error) {
	...
	logs.Info("ReadGoroutineNum is ", secLayerContext.secLayerConf.ReadGoroutineNum)
	...
	logs.Debug("all process goroutine existed")
	return
}
...
func HandleSecKill(req *SecRequest) (res *SecResponse, err error) {
	logs.Debug("this is HandleSecKill")
	...
	logs.Debug("this is HandleSecKill2")
	...
	logs.Debug("this is HandleSecKill200")
	histryCount := userHistory.GetCountByProduct(req.ProductId)
	secLayerContext.HistoryMapLock.Unlock()
	logs.Debug("this is HandleSecKill20077")
	...
	logs.Debug("this is HandleSecKill211")
	if curSoldCount >= product.Total {
		res.Code = ErrSoldout
		product.Status = ProductStatusSoldout
		return
	}
	logs.Debug("this is HandleSecKill3")
